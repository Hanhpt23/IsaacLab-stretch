...
class StretchReachCustomEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: StretchReachCustomEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.terminated_idx = self._robot.body_names.index('link_mast')
        ...
        self.robot_dof_speed_scales[self._robot.find_joints("joint_left_wheel")[0]] = 10.0
        self.robot_dof_speed_scales[self._robot.find_joints("joint_right_wheel")[0]] = 10.0
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # print('Action robot_dof_targets: ', self.robot_dof_targets)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # print('Body quaternion: ',self._robot.data.body_quat_w[:, self.terminated_idx][:, 1])

        # Additional termination condition: if the body quaternion of 'link_mast' exceeds a specific threshold
        body_quat_threshold = 0.6
        terminated_quat_condition = self._robot.data.body_quat_w[:, self.terminated_idx][:, 1] < body_quat_threshold

        # Combine both termination conditions
        terminated = terminated | terminated_quat_condition
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # print('Default position: ',self._robot.data.default_joint_pos[env_ids] )
        # pass
        # robot state
